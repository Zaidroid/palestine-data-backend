...
/**
 * Process Prisoners Data (Addameer monthly time series)
 *
 * Source file: public/data/static/prisoners-addameer.json — generated by
 * scripts/fetch-prisoners.js. One row per (month, metric_type) where
 * metric_type ∈ {total, administrative, child, female}.
 */
async function processPrisonersData() {
    logger.info('Processing prisoners data...');
    try {
        ...
        if (records.length === 0) {
            logger.warn('No Addameer prisoner records to process');
            return;
        }

        const { PrisonersTransformer } = await import('./utils/prisoners-transformer.js');
        const transformer = new PrisonersTransformer();
        const pipeline = new UnifiedPipeline({ logger });
        const outputDir = path.join(UNIFIED_DIR, 'prisoners');

        const results = await pipeline.process(
            records,
            { source: 'Addameer', organization: 'Addameer', category: 'prisoners' },
            transformer,
            { enrich: true, validate: true, partition: true, outputDir }
        );

        if (results.success) {
            await fs.mkdir(outputDir, { recursive: true });
            const enriched = results.enriched || [];
            const months = new Set(enriched.map((r) => r.date)).size;
            await fs.writeFile(
                ...
                JSON.stringify({
                    ...
                    metadata: {
                        ...
                        notice: 'Addameer publishes monthly aggregate prisoner counts; one record per (month, metric_type).',
                    },
                }, null, 2),
                'utf-8',
            );
            ...
            logger.success(`Processed ${enriched.length} prisoner records (${months} months)`);
        }
    } catch (e) {
        logger.warn('Error processing prisoners data:', e.message);
    }
}
