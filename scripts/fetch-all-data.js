...
async function saveTech4PalestineData(data) {
  ...
  if (data.killedInGaza && Array.isArray(data.killedInGaza)) {
    ...
    // Check if it's array of arrays (header + data)
    if (data.killedInGaza.length > 0 && Array.isArray(data.killedInGaza[0])) {
      const header = data.killedInGaza[0]; // ["id","en_name","ar_name","age","dob","sex","update"]
      const rows = data.killedInGaza.slice(1);
      ...
      records = data.killedInGaza;
    }

    // Filter? No date available, so assume all are relevant (post Oct 7)
  ...
  if (data.infrastructure && Array.isArray(data.infrastructure)) {
    ...
    for (const [quarter, quarterData] of Object.entries(quarters)) {
      await writeJSON(path.join(infraPath, `${quarter}.json`), {
        metadata: {
          source: 'tech4palestine',
      ...
      console.log(`  ✓ Saved infrastructure/${quarter}.json (${quarterData.length} records)`);
    }

    // Save index
    await writeJSON(path.join(infraPath, 'index.json'), {
      dataset: 'infrastructure',
      date_range: {
        start: normalized[0]?.date || BASELINE_DATE,
        end: normalized[normalized.length - 1]?.date || new Date().toISOString().split('T')[0],
    ...
    console.log('  ✓ Saved infrastructure/index.json');
  }
  ...
  await writeJSON(path.join(basePath, 'metadata.json'), {
    ...
    datasets: {
      ...
      infrastructure: data.infrastructure ? data.infrastructure.length : 0,
    },
  });
...
async function calculateStatistics() {
  ...
  const sources = ['hdx', 'tech4palestine', 'goodshepherd', 'worldbank', 'who', 'pcbs'];
  let totalDatasets = 0;
  let totalRecords = 0;
  let totalSize = 0;
...
async function updateManifest() {
  ...
  const sources = ['hdx', 'tech4palestine', 'goodshepherd', 'worldbank', 'who', 'pcbs', 'btselem'];
...
async function main() {
  ...
  try {
    ...
    const fetchScripts = [
      ...
      {
        name: 'Water (WASH)',
        path: path.join(__dirname, 'fetch-water-data.js'),
        description: 'Water & Sanitation Data Collection',
        required: false,
      },
      {
        name: 'Infrastructure',
      ...
      {
        name: 'News',
        path: path.join(__dirname, 'fetch-rss-feeds.js'),
        description: 'News Feed Aggregation',
        required: false,
      },
      {
        name: 'Historical',
        path: path.join(__dirname, 'fetch-historical-data.js'),
    ...
    const scriptPromises = fetchScripts.map(async (script) => {
      ...
      return executeScript(script.name, script.path, script.description);
    });
