...
_TYPE_FIELDS = ["Type", "type", "OBSTACLE", "obstacle_type", "ObstacleType", "Category"]
    ...
    try:
        ...
        for feat in fc.get("features", []):
            ...
            conn.execute(
                ...
                "VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
                (nid, "checkpoint", _slug(otype), name, props.get("name_ar") or name,
                 props.get("Governorate") or props.get("governorate"), pt[0], pt[1], "ocha_hdx", None, None, f"OCHA obstacle type: {otype}"),
            )
            if nid not in existing:
                added += 1
                existing.add(nid)
            append_fact(conn, "node", nid, "exists", True, "ocha_hdx", "node_exists_permanent", stamp, f"OCHA HDX survey ({otype})")
        conn.commit()
